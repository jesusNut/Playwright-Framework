...
test("Validate user able to sign in successfully with valid credentials & self heal",{tag : ['@regression']}, async ({
  landingPage,
  signinPage,
  homePage,
}) => {
...
loginData.forEach(data => {
  test(`Validate error messages while login with ${data.scenario}`,{tag : ['@regression']}, async ({
    landingPage,
    signinPage,
  }) => {
    await test.step(`Navigated to Application`, async () => {
