...
test("Validate the title of landing page",{tag : ['@smoke','@regression']}, async ({ landingPage }) => {
...
test("Validate the Header Text of landing page",{tag : ['@smoke','@regression']}, async ({ landingPage }) => {
  await test.step(`Navigated to Application`, async () => {
    await landingPage.visit();
  });
  await test.step(`Validated that correct header text is displayed on Landing page`, async () => {
    await landingPage.validateHeaderText("conduit");
                       ...
                       test("Validate the Sidebar text of landing page",{tag : ['@smoke','@regression']}, async ({ landingPage }) => {
  await test.step(`Navigated to Application`, async () => {
    await landingPage.visit();
  });
...
test("Validate promotion page URL is opening ",{tag : ['@regression']}, async ({ landingPage }) => {
  await test.step(`Navigated to Application`, async () => {
    await landingPage.visit();
  });
